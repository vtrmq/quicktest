...
let audioId = $state() as HTMLAudioElement;
let audio = $state('');
let isPlay = $state(false);
let downloadLink = $state() as HTMLAnchorElement;
...
function startRecording(stream: any) {
  ...
  mediaRecorder.onstop = () => {
    ...
    audioId.src = URL.createObjectURL(audioBlob);
    downloadLink.href = audioId.src;
    audio = `${crypto.randomUUID()}.ogg`;
  };
...
function audioEnd() {
  isPlay = false;
  audioId.removeEventListener('ended', audioEnd);
}

function start() {
  if (audioId.src.length === 0) return;
  isPlay = true;
  audioId.play();
  audioId.addEventListener('ended', audioEnd);
}
...
<style>
  ...
  .recorder {
    background: #07b307;
    box-shadow: #0b7310 0px 3px 0px 0px;
    color: #fff;
  }
  .play {
    background: #3e9eeb;
    box-shadow: #005da7 0px 3px 0px 0px;
    color: #fff;
  }
  .download {
    background: #d07e05;
    box-shadow: #9d5e00 0px 3px 0px 0px;
    color: #fff;
  }
</style>
