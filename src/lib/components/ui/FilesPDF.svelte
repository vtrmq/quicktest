...
async function handleChange(event: Event) {
  ...
  if (files && files.length > 0) {
    ...
    const new_file = `${crypto.randomUUID()}.${files[0].type.split('/')[1]}`;
    const formData = new FormData();
    formData.append('file', files[0]);
    formData.append('fileName', new_file);
    btnSave?.load(true);
    ...
    btnSave?.load(false);
    filesPDF = result.files;
  }
}

function handleSelectFile(index: number) {
  handleShowAlbum(false);
  onSelectFile(filesPDF[index].name, filesPDF[index].shadow_file);
}
...
async function handleActionDelete(e: string) {
  if (e === 'accept') {
    const fileId = filesPDF[posFile].file_id.toString();
    const shadowFile = filesPDF[posFile].shadow_file;
    const response = await fetch('/api/files', {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ fileId, shadowFile })
    });
    const result = await response.json();
    if (result.success === 'failed') {
      goto('/');
...
function handleSelectFileDelete(index: number) {
  posFile = index;
  dialog?.show({
    type: 'delete',
    message: '¿Quieres eliminar el archivo?',
  });
}
...
{#if isDisplay}
  <input class="hidden" bind:this={fileInput} type="file" accept="application/pdf" onchange={handleChange} />
      ...
      {:else if type === 2}
        ...
        <div>
          <div class="container-btns-album">
            <Title --color-title="#fff">Lista de archivos PDF</Title>
          ...
          {#if filesPDF.length === 0}
            <div class="album-empty">No tienes archivos</div>
          {:else}
            <div class="images-wrapper">
              <div class="container-images">
                {#each filesPDF as fl, index}
                  <div class="box-image" role="button" tabindex="0" onclick={()=>handleSelectFile(index)} onkeyup={()=>{}}>
                    <div class="wr-box-btns-actions" onclick={(e)=>e.stopPropagation()} role="button" tabindex="0" onkeyup={()=>{}}>
                      <button class="btn-action" onclick={()=>handleSelectFileDelete(index)}>{@html trash}</button>
                    </div>
                    <p class="image">{fl.name}</p>
                  </div>
                {/each}
              </div>
            </div>
          ...
          <div class="container-btns-image">
            <Button icon={upload} bg="green" onclick={() => fileInput?.click()} bind:this={btnSave} align="auto">Subir archivo PDF</Button>
          </div>
        </div>
      {/if}
    </div>
