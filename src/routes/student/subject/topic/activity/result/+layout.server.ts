...
export const load: LayoutServerLoad = async ({ locals, url, platform }) => {
  ...
  const [topic, activity, scales, answer] = await db.batch([
    db.prepare("SELECT topic FROM topics WHERE topic_id = ?").bind(topicId),
    db
      .prepare(
        "SELECT * FROM activities WHERE activity_id = ? AND topic_id = ?",
      )
      .bind(activityId, topicId),
    db
      .prepare(
        "SELECT scale, min_value, max_value FROM scales WHERE teacher_id = ?",
      )
      .bind(teacherId),
    db
      .prepare(
